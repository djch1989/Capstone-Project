...
Predict <- function(x) {
    ...
    if (length(inputtext)>= 3) {
        ...
        else {message_text <<- "N Gram used to predict: 4-gram."; head(quadgram[quadgram$unigram == inputtext[1] & quadgram$bigram == inputtext[2] & quadgram$trigram == inputtext[3], 4],1)}
    }
        ...
        else {message_text <<- "N Gram used to predict: 3-gram."; head(trigram[trigram$unigram == inputtext[1] & trigram$bigram == inputtext[2], 3],1)}
    }
        ...
        else {message_text <<- "N Gram used to predict: 2-gram."; head(bigram[bigram$unigram == inputtext[1],2],1)}
    }
}
